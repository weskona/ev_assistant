...
| `range_estimate` | Estimated Range | Current SoC × usable battery capacity ÷ actual consumption (kWh/100 km) — using your rolling 30-day/50 km trip-log average where available, falling back to the lifetime `vehicle_avg_consumption` otherwise. Attribute `verbrauch_kwh_100km` shows the consumption figure used. `unknown` without SoC or any consumption data. |
