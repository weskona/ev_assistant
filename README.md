...
- **Away only**: the evcc/wallbox and charge-power setup steps are skipped, and the Overview (Beta) panel tab shows only what's relevant to a pure external charger — the last confirmed external charge instead of the wallbox/SoC cards, an AC/DC split instead of a home/external location breakdown, spending over time, total kWh/cost, EUR/100 km, and the combustion-vehicle cost/CO2 comparison (which would otherwise be empty or trivially "100% external"). Switching **to** "Away only" doesn't delete any already-configured evcc/wallbox values, it just stops showing/using them; switching back to "Mixed"/"Home only" brings the same steps back with whatever was there before.
...
The day-ahead baseline compares the current battery against tomorrow's typical usage (plus whatever's left of today), re-evaluated roughly every minute and **written whenever it differs from evcc's actual live state** — not just from what ev_assistant itself last wrote, so it self-heals if evcc silently loses the setting again (e.g. an evcc add-on restart falling back to its own config defaults — without this, the weekly full charge below could stay permanently blocked, since the vehicle would never actually reach 100% again). One consequence: a manual override made directly in evcc's own UI now only survives until the next cycle (~1 min), not indefinitely — use the `set_evcc_mode_control_pause` service (also exposed as a toggle right in the "Automatische Ladesteuerung" panel card, see [Services](#services) below) to hold off write-back until you switch it back off, when you need a deliberate manual change to stick for a while:

- **`pv`** (pure solar charging) — the current battery already covers the rest of today plus tomorrow's typical usage.
- **`now`** (force grid charging) — it doesn't, so evcc is told to charge now regardless of solar.

This used to be a three-tier decision with a distinct `minpv` middle tier gated by a rolling 1-2 day buffer (comparing the battery against tomorrow *and* the day after) — production incident: it would force overnight grid pre-charging for a shortfall entirely against the day-after-tomorrow's forecast, even though tomorrow's own need was already covered and that second day's forecast gets reassessed with fresher data once it's actually next. The buffer window was reduced to just tomorrow, at which point it coincides exactly with the baseline's own single-day comparison, collapsing the decision to the two-way `pv`/`now` choice above. `minpv` is still a mode you'll see written to evcc, just from elsewhere: the real-time PV-surplus override below, the weekly full-charge override, or an active charge plan.

Example: usage profile says 8 kWh/day, buffer 20%, 45 kWh usable, 40% SoC (18 kWh) available. Required = 8 × 1.2 = 9.6 kWh → 18 kWh already covers it, so evcc gets `pv` and no min-/target-SoC override is needed from this mechanism.
...
Any leftover PV surplus forecasted for **today**, beyond what today itself needs, also reduces tomorrow's requirement (`min_kwh`/`target_kwh`) — not just today's own figure. Without this, an unusually high-consumption tomorrow (e.g. a planned trip) could force overnight grid pre-charging well in advance, even on a day with abundant solar forecast that alone would cover it (production incident: the vehicle charged from the grid all night for an upcoming high-usage day, despite a full day of solar ahead that would have covered it). The `evcc_mode_control` sensor's `pv_ueberschuss_puffer_kwh` attribute shows how much surplus was credited this way.
...
This solves a case the profile-based mode control above doesn't: a hard deadline. The `pv`/`now` day-ahead decision only looks at tomorrow's typical usage, never a specific clock time.
...
**"Wirtschaftlichkeit Netz-Zuschuss" card**: visualizes the [economic cap on the real-time PV surplus override](#automatic-evcc-mode--soc-control) (`evcc_realtime_override_min_solar_share`) as a scale from the feed-in tariff to the grid price, with markers for the current blended price and the configured threshold — shown whenever evcc reports live tariffs, independent of whether the cap itself is configured.
...
|---------|-----------|-------------|
| `set_usage_profile_buffer_pct` | `config_entry_id`, `buffer_pct`* | Runtime override for the safety buffer % used by the [usage profile](#automatic-evcc-mode--soc-control)/"charge before PV" recommendation, without reopening the options flow (also the panel's buffer slider). Omit `buffer_pct` to reset to the configured value. |
| `urlaub_seit` | `config_entry_id`, `seit_ts` | Retroactively re-books already-confirmed Live-SoC discharge bookings since the given Unix timestamp back out of their weekday's usage-profile window — for when vacation mode was only switched on after departure. Only affects recent days still in the discharge-events log. |
| `set_weekly_full_charge_enabled` | `config_entry_id`, `enabled`* | Toggles the [weekly full-charge balancing](#automatic-evcc-mode--soc-control) feature directly (e.g. the panel switch) — still persisted to config (a brief integration reload), so the options flow never shows a stale value. Omit `enabled` to reset to the default (off). |
| `set_evcc_mode_control_pause` | `config_entry_id`, `paused` | Suppresses automatic mode/min-/target-SoC write-back to evcc while on — e.g. while you're changing something directly in evcc's own UI and don't want it overwritten next cycle (~1 min). Stays on until called again with `paused: false` (also the panel's pause toggle on the "Automatische Ladesteuerung" card). |
| `set_evcc_charge_plan` | `config_entry_id`, `target_soc`, `target_time` | Sets evcc's own [charge plan](#charge-plan) (target-time charging). Pauses the profile-based mode/SoC control automatically while active. |
| `clear_evcc_charge_plan` | `config_entry_id` | Removes an active evcc charge plan — profile-based mode/SoC control resumes automatically (if enabled and not manually paused). |
| `set_evcc_charge_plan_range_km` | `config_entry_id`, `target_range_km`, `target_time` | Same as `set_evcc_charge_plan`, but takes a target remaining range in km instead of a target SoC % — converted internally using the current average consumption (the same figure used for the range display). |
| `set_evcc_manual_mode` | `config_entry_id`, `mode` | Sets evcc's charge mode directly (`pv`/`minpv`/`now`) and pauses the profile-based mode/SoC control until the vehicle disconnects (session-scoped, resets automatically). Min-/target-SoC are left untouched. |
| `clear_evcc_manual_mode` | `config_entry_id` | Ends a manual mode set via `set_evcc_manual_mode` early, without waiting for disconnect — profile-based control resumes immediately. |
...
| `set_panel_layout` | `config_entry_id`, `layout` | Saves Beta-panel card visibility/order — called by the panel's own "Anpassen" popup, not intended for manual use. |
