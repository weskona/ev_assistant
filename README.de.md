...
Rein additiv und komplett im Panel verwaltet (kein Config-Flow-Schritt) — siehe den Abschnitt „Wartung-Tab" unten. Verwaltet wiederkehrende Wartungspunkte (HU/TÜV, Inspektion, saisonaler Reifenwechsel, Bremsflüssigkeit, ...) mit bis zu drei unabhängigen Fälligkeitskriterien pro Punkt — Kilometer-Intervall, Zeit-Intervall (Tage) und/oder festes Datum — je nachdem, was zuerst eintritt.

| Key | Name | Beschreibung |
|-----|------|--------------|
| `wartung_faellig` | Wartung fällig | Anzahl bald fälliger oder überfälliger *aktiver* Punkte (pausierte zählen nicht mit). `unknown` ohne konfigurierten Punkt. Attribut `punkte`: die vollständige Liste, je mit den eigenen Feldern des Punkts plus dem berechneten `status` (`ok`/`bald_faellig`/`ueberfaellig`/`unbekannt`), `naechste_quelle` (welches Kriterium gerade gewinnt), `rest_tage`/`faellig_datum` (vom jeweils frühesten Kriterium — das km-Kriterium liefert erst dann ein Datum, wenn das aktuelle Fahrtempo, ein rollierender 30-Tage-Schnitt, bekannt ist), und `rest_km` (der Kilometer-Rest des km-Kriteriums, unabhängig davon, ob es das gewinnende Kriterium ist — ein zeitbasierter Punkt mit zusätzlichem km-Intervall zeigt also beides). `presets`: die eingebauten Start-Vorlagen (`tuev`, `inspektion`, `reifenwechsel_saisonal`, `innenraumfilter`, `bremsfluessigkeit`, `reifenalter`) im Anlegen-Formular des Panels — füllen nur vor, bleiben frei überschreibbar, keine herstellerspezifischen Werte. |
...
| `add_maintenance` | `config_entry_id`, `name`*, `preset`*, `km_intervall`*, `zeit_intervall_tage`*, `festes_datum`*, `kosten`*, `last_done_km`*, `last_done_datum`* | Legt einen neuen Wartungspunkt an (siehe [Fahrzeugwartung](#fahrzeugwartung) oben). `preset` (eines von `tuev`, `inspektion`, `reifenwechsel_saisonal`, `innenraumfilter`, `bremsfluessigkeit`, `reifenalter`) füllt `name`/Intervall-Standardwerte als Startpunkt — jedes explizit angegebene Feld gewinnt. Braucht einen Namen (eigenen oder aus der Vorlage) und nach Anwendung der Vorlage mindestens ein Fälligkeitskriterium (km-Intervall, Zeit-Intervall und/oder festes Datum), sonst wird der Aufruf abgelehnt und protokolliert. |
| `edit_maintenance` | `config_entry_id`, `wartung_id`, `name`*, `km_intervall`*, `zeit_intervall_tage`*, `festes_datum`*, `kosten`*, `last_done_km`*, `last_done_datum`*, `aktiv`* | Korrigiert ein beliebiges Feld eines bestehenden Punkts. Nur angegebene Felder ändern sich; ein leeres `km_intervall`/`zeit_intervall_tage`/`kosten`/`festes_datum` löscht dieses Feld. Wird abgelehnt (und zurückgerollt), falls dadurch kein Fälligkeitskriterium mehr übrig bliebe — ein Punkt braucht immer mindestens eines. `aktiv: false` pausiert den Punkt (zählt nicht mehr in `wartung_faellig`, bleibt aber im Panel sichtbar). |
