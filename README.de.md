...
| 3 — evcc & Wallbox | Die Adresse des evcc-Addons (z.B. `http://192.168.178.1:7070`, optional — spricht evccs REST-API direkt an, keine separate HA-Integration nötig), der Fahrzeugname in evcc (für den Heimladen-Historienfilter) und die Wallbox-Ladeleistungs-Entität (dient als Heimlade-Signal — jeder Wert > 0,1 kW gilt als „lädt zuhause"). Ein Zusatzschritt erscheint nur, wenn evcc mehr als einen Ladepunkt verwaltet, um den zu diesem Fahrzeug gehörenden auszuwählen. **Übersprungen** im Modus „Nur auswärts". |
