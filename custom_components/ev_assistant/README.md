...
It's a three-tier decision, re-evaluated roughly every minute but only ever **written when the recommendation actually changes** — a manual override in evcc's own UI (e.g. forcing `now` yourself) sticks around until the next real change, it's never silently reverted:
...
Die Entscheidung erfolgt dreistufig, wird etwa minütlich neu berechnet, aber **nur bei tatsächlicher Änderung der Empfehlung geschrieben** — ein manueller Eingriff direkt in evccs eigener Oberfläche (z.B. selbst auf `now` stellen) bleibt bis zur nächsten echten Änderung bestehen und wird nicht stillschweigend überschrieben:
