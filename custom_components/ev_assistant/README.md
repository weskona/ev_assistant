...
| `evcc_mode_control` | evcc-Modus-Steuerung | Diagnose-Sensor für die [automatische evcc-Modus-/SoC-Steuerung](#automatische-evcc-modus-soc-steuerung) unten — der Modus (`pv`/`minpv`/`now`), auf den gerade hingesteuert wird, bzw. `unknown`, wenn diese Funktion aus ist (Standard) oder noch kein Nutzungsprofil vorliegt. Attribute: `min_soc`, `target_soc`, `verfuegbare_kwh`, `min_kwh`, `target_kwh`, `rest_heute_kwh`, `rest_heute_roh_kwh`, `pv_rest_heute_roh_kwh`, `haus_rest_heute_kwh`, `pv_fuer_auto_kwh`, `zuletzt_geschrieben`, `aktiv`, `min_soc_scope`, `limit_soc_scope`, `balancing_enabled`, `balancing_aktiv`, `naechste_vollladung_faellig_ts` (siehe [wöchentliche Vollladung](#automatische-evcc-modus-soc-steuerung) unten). |
