...
2. **Charging Mode** (optional) — how you charge this vehicle: **Home only**, **Mixed** (default), or **Away only**. Purely controls which of the following steps and which panel tabs/cards appear — never the underlying calculations (a pure external charger is simply the case where the home aggregates are 0). Changeable at any time without losing already-entered data: switching to "Away only" just stops showing/using the home-related steps/cards, it doesn't delete them.
3. **evcc & Wallbox** (optional) — the evcc add-on's host (e.g. `http://192.168.178.1:7070`) — EV Assistant talks directly to evcc's own REST API, **no separate `evcc_intg` HA integration needed**. The vehicle name in evcc (filters the home-charging history to this vehicle when multiple EVs share one evcc instance) and the **wallbox charge power entity** (filtered to `sensor` + `device_class: power` — used as the yes/no signal "currently charging at home"; a value **above 0.1 kW counts as "charging"**) are optional too. Without the wallbox charge power entity every SoC increase is falsely detected as an external charge. A follow-up step appears only if evcc manages more than one charge point, to pick which one belongs to this vehicle. Also here, all opt-in and off by default: `evcc_mode_control_enabled` (see "Automatic evcc mode/SoC control" below), `evcc_battery_priority_enabled` (that control's home battery priority override), `wallbox_min_power_w` (that control's real-time PV surplus override, default 1380 W), `evcc_realtime_override_min_solar_share` (an optional minimum solar share, %, on that same override — the equivalent price cap is computed live from evcc's own tariffs, not stored as a fixed price), `pv_forecast_today_remaining_entity` (today's remaining PV forecast, for that control — a different entity/timespan than `pv_forecast_entity` in step 7), and the optional house usage profile's `home_consumption_entity`/`battery_charge_entity`. **Skipped** in "Away only" mode.
4. **Charging power** (optional) — two different sources for two different purposes. Charging power is a momentary power reading in W/kW, typically from vehicle telemetry rather than your wallbox (so it still reports data during an external charge, where the wallbox is idle); it improves the energy estimate of an external charge beyond plain SoC-delta (see "Energy estimation methods" above). The **wallbox energy meter**, on the other hand, is a cumulative kWh counter of your own wallbox (never momentary, only relevant while charging at home) — it's used for automatic efficiency calibration *and* for the home-charging cost tracked in step 9. **Skipped** in "Away only" mode.
5. **Notifications** (optional) — a persistent notification in Home Assistant's own notification panel always appears automatically for a detected external charge, trip, or Tankerkönig outage, regardless of this step. Pick one or more `notify.*` target devices (a multi-select entity picker — phone, tablet, email, ...) and which of those events should additionally send a push (e.g. to your phone): external charge detected, trip detected, Tankerkönig unavailable, and/or SoC threshold reached. If "SoC threshold reached" is selected, choose which percentages (50/60/70/80/90/100%) to be notified at — a notification fires once per threshold as the battery crosses it during any charging session in progress, home or external.
6. **Detection fine-tuning** — thresholds of the underlying state machine described above (`start_delta`, `noise`, `idle_timeout_s`, `drop_ends`). A session starts once SoC rises by `start_delta` above its last resting value; it ends either when SoC drops by more than `drop_ends` below the tracked peak, or when `idle_timeout_s` passes with no new peak (e.g. battery full). `noise` tolerates ordinary sensor jitter while tracking the peak and must always be **smaller** than `start_delta`, or jitter alone could trigger a false detection. Defaults work for most vehicles; raise them for a car whose SoC only updates coarsely/infrequently (e.g. some cloud APIs). Two optional fields, `plug_entity` (a plug/connectivity `binary_sensor`) and `plug_debounce_s`, offer a more direct alternative for exactly that case: a confirmed "plugged in" overrides `idle_timeout_s` entirely (the session simply never times out while the car stays connected), and a confirmed "unplugged" — held for `plug_debounce_s` first, to ride out brief flaky readings from the source — ends the session immediately. A confirmed "unplugged" also prevents a session from *starting* in the first place: a SoC rise while the vehicle is confirmed unplugged (e.g. a few points recovered via regenerative braking while driving) just moves the tracked reference point up instead of being misread as the start of an external charge. Even without `plug_entity`, a newly detected charge is merged into the previous still-open one whenever there was no SoC drop in between, so a coarse-SoC vehicle tripping `idle_timeout_s` mid-charge no longer shows up as several separate pending charges.
7. **Trip log** (optional) — thresholds of the trip detector (`trip_min_km`, `trip_idle_timeout_s`) described in "Trip log" below. Defaults work for most odometer sensors. Also an optional location-suggestion source (`gps_entity`, a `person`/`device_tracker`/`sensor` entity — for a plain `sensor`, its state is used directly as the suggested location if it doesn't match an existing zone). A third optional pair, `motor_entity` (a motor/driving `binary_sensor`, e.g. ignition/"Ready") and `motor_debounce_s`, gives the trip detector a second signal for vehicles whose odometer updates too coarsely/infrequently to reliably derive trip start/end from standstill alone: a confirmed "driving" state starts/continues a trip even without a fresh odometer reading, and a confirmed "not driving" is treated like "no odometer movement" — `trip_idle_timeout_s` still tolerates brief stops (e.g. stop-start at a light), so a momentary motor-off reading mid-trip doesn't end it. Distance always comes from the odometer regardless of this signal; a genuine odometer increase is never ignored even if the motor signal disagrees. A further optional toggle, `trip_auto_confirm`, skips the manual confirmation step entirely: a detected trip is added to the trip log immediately instead of sitting as a "pending" entry waiting for a start/end location. The location comes from the `gps_entity` suggestion if configured, otherwise it stays empty — still correctable afterwards via `edit_trip`. One more optional field, `usage_profile_buffer_pct` (default 20), sets the safety margin added on top of the historical weekday average for the "Usage Profile" panel tab's "needed tomorrow" figure — see "Usage profile" below. A further optional field, `pv_forecast_entity`, points at any sensor entity providing tomorrow's solar-yield forecast (e.g. from Solcast or Forecast.Solar, in kWh or Wh) — with it configured, the charge recommendation lets tomorrow's expected PV generation cover a shortfall the battery alone can't.
8. **Leasing** (optional) — only active once **both** `leasing_inkl_km` and `leasing_end_datum` are set: contract starting odometer reading (`leasing_start_km`), contract start/end date, total included mileage, and an optional per-km price for overage (`leasing_preis_mehr_km`) and/or credit for underage (`leasing_preis_minder_km`). Leave it empty and the feature stays fully inactive — no sensor state, no panel content.
9. **Cost comparison** (optional) — see "Cost comparison vs. a combustion car" below. Fuel price priority: Tankerkönig auto-detection (pick a fuel type) > fuel-price entity > fixed value — configure only one source. Also optional: `co2_per_kwh_g` (grid CO2 intensity in g/kWh, default 380 — a rough German-grid-average estimate) for the `CO2 Savings vs. ICE Vehicle` sensor described below.
...
All fields in step 9 are optional — configure as many or as few as you like; the sensors below simply show `unknown` until their required inputs are available.
...
| `sensor ... External Charge Cost (Last)` | `kwh × price_kwh` for that same most recent confirmed charge, plus its `start_fee` if any (`engine.charge_cost()`). Its `historie` attribute (full history list) and each entry within it carry the raw `startgebuehr` value too — 0 for entries confirmed before this field existed or where none was entered. |
...
| `sensor ... CO2 Savings vs. ICE Vehicle` | Estimated CO2 saved vs. the reference combustion car, same energy balance and km-driven basis as `... Vehicle Avg Consumption`: `(combustion fuel use × its CO2 factor) − (EV kWh used × co2_per_kwh_g)`. The combustion CO2 factor is a fixed constant per fuel type (2.33 kg/L super, 2.24 kg/L super E10, 2.65 kg/L diesel — tank-to-wheel, no upstream/production emissions), taken from `tankerkoenig_fuel_type` if set, otherwise defaulting to super/gasoline. `unknown` until the odometer entity and combustion consumption are configured. Attributes: `co2_ev_kg`, `co2_verbrenner_kg`, `co2_ersparnis_kg`. |
| `sensor ... External vs. Home Charging Price Difference` | The weighted-average price actually paid for external charging (total confirmed external cost ÷ total confirmed external kWh) minus the home electricity price from step 9 — both in €/kWh, since setup. Positive means external charging cost more per kWh, which is the usual case. `unknown` without a resolvable home electricity price, or before any external charge has been confirmed. Attributes: `heimladen_preis_kwh`, `fremdladen_preis_kwh`, `differenz_kwh`. |
| `sensor ... Cost (Today/Week/Month/Year)` (diagnostic) | Combined home + external charging cost accrued within the current calendar day/ISO week/month/year, using the same period-baseline-and-rollover mechanism as the driven-km sensors below (a baseline is captured once per period at the daily 00:05 refresh, or immediately at first startup; the sensor shows current total minus that baseline). `unknown` before the first baseline is captured. Unlike the km period sensors, a dip below the baseline is clamped to 0 rather than shown as `unknown` — the home-cost estimate's weighted-average price can tick down slightly as a cheaper session is folded in, which isn't a data error the way a shrinking odometer would be. |

### Usage profile

Answers "do I need to charge tonight, or can charging wait for tomorrow's solar surplus?" purely from the trip log, no manual input needed. `weekday_usage_profile()` in `engine.py` buckets every logged trip's `verbrauch_kwh` (or, if unknown, `km` × `vehicle_avg_consumption` as an estimate) by weekday and averages it over the number of times that weekday has occurred since the first logged trip — days without a trip still count as 0 kWh for that weekday, so a car rarely driven on Sundays correctly pulls the Sunday average down instead of that day being skipped. Needs at least 7 days of trip-log history (`MIN_USAGE_PROFILE_DAYS`) before returning anything, so every weekday has been observed at least once.

| Key | Name | Description |
|-----|------|-------------|
| `usage_profile` | Usage Profile | Average kWh consumed on today's weekday. Attributes: `montag`…`sonntag` (all 7 weekday averages, in kWh). `unknown` with less than 7 days of trip-log history. |
| `usage_profile_tomorrow` | Usage Profile (Needed Tomorrow) | Tomorrow's weekday average plus the `usage_profile_buffer_pct` safety margin from step 7 (default 20 %) — directly comparable to `available_kwh`. Attributes: `wochentag`, `roh_kwh` (unbuffered), `puffer_prozent`, `benoetigt_kwh` (same as the state). `unknown` under the same condition as `usage_profile`. |
...
| `plug_window` | Plug Time Window | Observation-only diagnostic: average hours per weekday the vehicle is plugged in at the wallbox, from evcc's own loadpoint `connected` state (not the separate, optional `plug_entity`). Attributes: `montag`…`sonntag` (only actually-observed weekdays), `heute` (today's still-running tally), `tage` (the raw recent-days window per weekday: `date`, `stunden`, `erster_connect`, `letzter_disconnect`). Doesn't influence charge mode/SoC control. |
| `evcc_mode_control` | evcc Mode Control | Diagnostic sensor for the [automatic evcc mode/SoC control](#automatic-evcc-mode--soc-control) below — the mode (`pv`/`minpv`/`now`) it's currently steering toward (already including the real-time PV surplus override below), or `unknown` if that feature is off (default) or the usage profile isn't available yet. Attributes: `min_soc`, `target_soc`, `verfuegbare_kwh`, `min_kwh`, `target_kwh`, `rest_heute_kwh`, `rest_heute_roh_kwh`, `pv_rest_heute_roh_kwh`, `haus_rest_heute_kwh`, `pv_fuer_auto_kwh`, `pv_ueberschuss_puffer_kwh` (today's leftover PV surplus credited against tomorrow's/the buffer's requirement), `zuletzt_geschrieben`, `aktiv`, `min_soc_scope`, `limit_soc_scope`, `balancing_enabled`, `balancing_aktiv`, `naechste_vollladung_faellig_ts` (see [weekly full charge](#automatic-evcc-mode--soc-control) below), `pv_override_aktiv`, `pv_ueberschuss_w` (see real-time PV surplus override below), `pv_override_mischpreis_kwh` (blended EUR/kWh cost of that override's grid top-up), `pausiert` (manual write-back pause on/off, `set_evcc_mode_control_pause` service/panel toggle). |
| `evcc_charge_plan` | evcc Charge Plan | Diagnostic sensor mirroring evcc's own charging plan (target-time charging, see Charge Plan below) — target time as state, attributes `target_soc`, `projected_start`, `projected_end`, `aktiv`. Independent of `evcc_mode_control_enabled`, only needs `evcc_host`. |
| `available_kwh` | Available kWh | Current SoC (from `soc_entity`) × the usable battery capacity from step 1. `unknown` without a configured SoC source. |
| `binary_sensor ... Charge Before Solar Recommended` | **On** when `available_kwh` is below `usage_profile_tomorrow`'s buffered figure — i.e. charging now (e.g. from the grid) is advisable rather than waiting for tomorrow's solar surplus. With `pv_forecast_entity` configured (`_pv_forecast_tomorrow_kwh()`, auto-converts Wh to kWh), tomorrow's forecasted PV yield is added to `available_kwh` before this comparison (`engine.charge_before_pv_decision()`) — a raw, undiscounted figure, so it doesn't account for how much of that forecast the house itself will consume before the car gets a share. Attributes: `verfuegbare_kwh`, `benoetigt_morgen_kwh`, `pv_prognose_morgen_kwh` (only present if `pv_forecast_entity` is configured and resolvable). `unknown` if either underlying sensor is `unknown`. |

The sidebar panel's "Usage Profile" tab visualizes this as a bar chart (today's and tomorrow's weekday highlighted) plus a plain-language recommendation. Below it, the panel also shows the optional **House Usage Profile** as its own bar-chart card — see below.

### Automatic evcc Mode / SoC Control

An **opt-in** extension of the usage profile above (`evcc_mode_control_enabled` in step 3, off by default) that doesn't just *recommend* charging — it actually writes evcc's charge mode (`pv`/`minpv`/`now`) and the vehicle's min/target SoC, so the wallbox follows the usage profile without any manual mode-switching. Requires a configured `evcc_host` (step 3); a repair issue is raised if evcc's SoC scope (loadpoint- vs. vehicle-level, and independently so for min-SoC vs. target-SoC — some evcc versions support one at the loadpoint level and the other only per vehicle) can't be determined — the mode is still set in that case, just not that particular SoC limit.

It's a three-tier decision, re-evaluated roughly every minute and **written whenever it differs from evcc's actual live state** — not just from what ev_assistant itself last wrote, so it self-heals if evcc silently loses the setting again (e.g. an evcc add-on restart falling back to its own config defaults — without this, the weekly full charge below could stay permanently blocked, since the vehicle would never actually reach 100% again). One consequence: a manual override made directly in evcc's own UI now only survives until the next cycle (~1 min), not indefinitely — use the `set_evcc_mode_control_pause` service (also exposed as a toggle right in the "Automatische Ladesteuerung" panel card) to hold off write-back until you switch it back off, when you need a deliberate manual change to stick for a while:
...
The profile feeding this control (both vehicle and house) uses a **recency-weighted sliding window** rather than a lifetime average: each weekday keeps only the last 8 observed calendar days (`USAGE_PROFILE_WINDOW_DAYS`) instead of the full history since setup, so a genuinely changed usage pattern is reflected within weeks instead of years. It's also **time-of-day-aware**: the remaining need for today (`remaining_today_kwh()`) tapers the weekday average down proportional to how much of the day has already elapsed, before subtracting what's already been used today — so the recommended remaining need naturally approaches zero as the day progresses, rather than only ever moving by literal kWh subtraction against a flat full-day average. The vehicle side is sourced entirely from the live-SoC tracker described above, with no trip-log fallback for a weekday not yet observed (treated conservatively as 0 kWh need instead).

Any leftover PV surplus forecasted for **today**, beyond what today itself needs, also reduces `min_kwh`/`target_kwh` (tomorrow's/the buffer window's requirement), not just today's own figure — without this, a single unusually high-consumption day inside the buffer window (e.g. a planned weekend trip) could force overnight grid pre-charging well in advance, even on a day with abundant solar forecast that alone would cover the whole window. The `evcc_mode_control` sensor's `pv_ueberschuss_puffer_kwh` attribute shows how much surplus was credited this way.

**Optional house usage profile** (`home_consumption_entity`, `battery_charge_entity`, step 3): without it, a configured `pv_forecast_today_remaining_entity` would be credited entirely to the vehicle, overstating how much solar surplus is actually left over for it once the house (and an optional home battery, if its charging isn't already counted in the consumption meter) has taken its share. A cumulative home-consumption energy meter fixes that — the house's own typical remaining-today need is subtracted from the PV forecast first. Like the vehicle's usage profile, it builds up its own per-weekday average from scratch (a few days of history before it has any effect; silently 0 kWh until then), but — unlike the vehicle profile, which needs a full 7 days before showing anything — the panel's House Usage Profile card fills in day by day, with a never-yet-observed weekday shown as a dashed placeholder rather than a misleading 0 kWh bar.

**Optional weekly full charge for cell balancing** (`weekly_full_charge_enabled`, off by default, plus `weekly_full_charge_interval_days`, default 7, step 3): most EVs are healthiest spending most of their time in a mid-range SoC window, which is exactly what the profile-based control above aims for — but the battery management system still needs an occasional full charge to 100% to recalibrate its cell balancing. When enabled and at least the configured number of days have passed since the SoC last reached ~100% (98% is treated as "full" — many vehicles never report an exact 100%), this overrides the profile-based target with target SoC 100% / mode `minpv` until 100% is actually reached, then resets the counter. It doesn't matter *how* 100% was reached — a manual full charge resets the countdown exactly the same as a triggered one, so this never charges further than necessary. The switch can also be flipped directly from the Wallbox card in the panel (a runtime override, like the buffer slider — no integration reload) — the interval itself stays an options-flow-only setting.
...
**Real-time PV surplus override** (`wallbox_min_power_w`, step 3, default 1380 W = 6A × 230V single-phase — for a three-phase wallbox connection it'd be 6A × 3 × 230V ≈ 4140 W): the three-tier decision above is re-evaluated roughly once a minute from the usage profile, which is intentionally slow-moving (day-scale). On top of it, a second, faster layer reacts to the *current* PV surplus (site PV minus grid export, read from the same live evcc feed already polled for the panel — no extra evcc call) and can upgrade `pv` to `minpv` in real time, **never** the other way, and only while the usage profile itself recommends `pv`. The problem it solves: in pure `pv` mode, the wallbox only draws power once the surplus clears its own minimum charge power (`wallbox_min_power_w`) — a surplus just below that (e.g. 1200 W against a 1380 W floor) charges nothing at all and gets fed into the grid unused, even though `minpv` would happily take it (topped up with a small grid/battery contribution). Once the surplus clears the floor by a further 100 W margin, it switches back to `pv` — that margin (hysteresis) is what stops it flapping back and forth on a surplus that hovers right at the threshold (passing clouds, a kettle switching on). It deliberately does **not** count PV currently charging a home battery as "surplus" — that's already being put to good use (autarky/cell balancing) rather than wasted, and upgrading the wallbox against that would fight any battery-protection priority you've set up in evcc itself.

**Optional economic cap on the grid top-up** (`evcc_realtime_override_min_solar_share`, step 3, %, no default — the override above is unconditional on wattage alone until this is set): solar power used for charging isn't actually free — every kWh not exported instead costs you the feed-in tariff you gave up. Filling the gap up to `wallbox_min_power_w` with grid power therefore has a real blended cost (`blended_charge_price()`): `(surplus_w × feed-in tariff + grid_topup_w × grid price) ÷ wallbox_min_power_w`. Rather than entering that price directly, you set a minimum acceptable **solar share** (e.g. 50%) and the equivalent price ceiling (`min_solar_share_price_ceiling()`) is computed fresh on every cycle from evcc's own live tariffs (`tariffFeedIn`/`tariffGrid`) — so it stays correct automatically if your tariff changes, instead of going stale like a fixed price would. The override only upgrades to `minpv` when the blended cost implies at least that much solar share; otherwise it stays `pv` and the small surplus is fed in instead of buying expensive grid power to use it. The `evcc_mode_control` sensor's `pv_override_mischpreis_kwh` attribute shows the current blended price whenever a `minpv` candidate is present, even before you've configured a cap, so you can see what a sensible share would be.

**Optional home battery priority** (`evcc_battery_priority_enabled`, step 3, off by default): the setting above deliberately respects evcc's own home-battery priority threshold (`prioritySoc`) — but a `prioritySoc` fixed at 100% means the battery always charges to full *first* on any PV-surplus day, before the vehicle gets anything. If the vehicle leaves right around when the battery reaches 100%, it gets essentially no solar charge, and any further surplus that day goes to the grid instead of ever reaching it. When enabled, ev_assistant lowers evcc's site-wide `prioritySoc` while the vehicle is plugged in (`EvccClient.async_set_priority_soc()`), giving it full priority over the battery for PV surplus, and restores the threshold you had set once the vehicle disconnects — rather than resetting it to a fixed value, so a preference set directly in evcc's own UI is respected. Only changes how PV *charging* surplus is split between wallbox and battery; has no effect on battery *discharge* behavior, which stays entirely under whatever separate control (evcc's own battery mode, or your own automation) you already have for that.

### Charge Plan

A separate, independently usable complement to the automatic mode/SoC control above: sets evcc's own charging plan (target-time charging) via the `set_evcc_charge_plan` service (`config_entry_id`, `target_soc`, `target_time` as a Unix timestamp) or the "Ladeplan" panel card — "the car needs to be at 80% by 7am tomorrow." evcc then decides itself *when* to charge (tariff-/PV-optimized). Only needs `evcc_host` configured, no `evcc_mode_control_enabled`. Solves a case the profile-based control above doesn't cover: a hard deadline, vs. its rolling 1-2 day buffer.

**While a plan is active** (`evcc_charge_plan` sensor has a state), the automatic mode/SoC control steps back entirely and writes nothing — whether the plan was set through ev_assistant or directly in evcc's own UI. Resumes automatically once the plan is cleared (`clear_evcc_charge_plan` service or the panel's "Plan löschen" button) or expires on its own.
...
**Optional location suggestion:** configure a `person` or `device_tracker` entity (step 7, e.g. the driver's phone) and the pending trip gets a `start_ort_vorschlag`/`end_ort_vorschlag` attribute — the entity's HA zone (e.g. "Home") at the moment the trip started/ended. This is a **prefill suggestion only**, not an automatic entry — `log_trip` still requires an explicit call, so a phone that stayed home or a trip outside any configured zone just means an empty suggestion, not a wrong automatic log entry.
...
- `ev_assistant.log_charge` — `config_entry_id`, `kwh`, `price_kwh` (+ optional `start_ts`, `start_fee`): confirm a pending charge and write it to history. **More than one charge can be pending at once** (e.g. two charging stops on a road trip before you get around to confirming either) — `start_ts` picks which one; without it, the oldest is confirmed (FIFO). `start_fee` is an optional flat fee some networks/chargers add on top of the kWh price (e.g. a start or blocking fee from the receipt), added to the stored cost (`engine.charge_cost()`); default 0.
...
- `ev_assistant.edit_charge` — `config_entry_id`, `erfasst_ts` (+ optional `kwh`, `price_kwh`, `start_fee`, `start_ts`, `end_ts`, `soc_start`, `soc_end`): correct any field of an already-confirmed history entry (e.g. a typo noticed after the fact, or filling in SoC values), identified by its `erfasst_ts` attribute (see the `historie` attribute on the last-cost sensor, or the [EV Assistant Card](https://github.com/weskona/ev-assistant-card)'s History list) — same "only given fields change" model as `edit_trip`. Running totals are adjusted by the difference (not recomputed from scratch) when kWh/price/fee change — `kosten` is always re-derived from the effective (new-or-existing) kWh/price/fee via `engine.charge_cost()`. Changing `soc_start`/`soc_end` recomputes `delta_soc`. `end_ts` isn't stored directly — it's converted to `dauer_min` together with the (new or existing) `start_ts`, same as at confirmation time.
...
- `ev_assistant.edit_trip` — `config_entry_id`, `erfasst_ts` (+ optional `start_ort`, `end_ort`, `start_ts`, `end_ts`, `km`, `odo_start`, `odo_end`, `soc_start`, `soc_end`, `verbrauch_kwh`): correct any field of an already-confirmed trip log entry (e.g. a typo, a wrong date/time, or a mis-detected distance), identified by its `erfasst_ts` attribute. Only given fields change; changing `km` adjusts the running trip-log total by the difference, changing `start_ts` also updates the entry's `datum` (used by the CSV export), and changing `soc_start`/`soc_end` recomputes `delta_soc` and — unless `verbrauch_kwh` is given explicitly — `verbrauch_kwh` too.
...
custom_components/ev_assistant/
  ...
  evcc_client.py      # evcc REST client (reads + evcc mode/SoC control writes)
  sensor.py
  binary_sensor.py
  services.yaml
  strings.json
...
2. **Lademodus** (optional) — wie du dieses Fahrzeug lädst: **Nur zuhause**, **Gemischt** (Standard) oder **Nur auswärts**. Steuert ausschließlich, welche der folgenden Schritte und welche Panel-Tabs/Karten erscheinen — nie die Berechnungen selbst (ein reiner Fremdlader ist rechnerisch einfach der Fall, in dem die Heim-Aggregate 0 sind). Jederzeit änderbar, ohne bereits erfasste Daten zu verlieren: ein Wechsel zu „Nur auswärts" blendet die Heim-bezogenen Schritte/Karten nur aus, löscht sie nicht.
3. **evcc & Wallbox** (optional) — die Adresse des evcc-Addons (z.B. `http://192.168.178.1:7070`) — EV Assistant spricht evccs eigene REST-API direkt an, **keine separate `evcc_intg`-HA-Integration nötig**. Ebenfalls optional: der Fahrzeugname in evcc (filtert die Heimladen-Historie auf dieses Fahrzeug, wenn mehrere EVs eine evcc-Instanz teilen) und die **Wallbox-Ladeleistungs-Entität** (gefiltert auf `sensor` + `device_class: power` — dient als Ja/Nein-Signal „lädt gerade zuhause"; ein Wert **über 0,1 kW gilt als „lädt"**). Ohne die Wallbox-Leistungsentität wird jeder SoC-Anstieg fälschlich als Fremdladung erkannt. Ein Zusatzschritt erscheint nur, wenn evcc mehr als einen Ladepunkt verwaltet, um den zu diesem Fahrzeug gehörenden auszuwählen. Ebenfalls hier, alle optional zuschaltbar und standardmäßig aus: `evcc_mode_control_enabled` (siehe „Automatische evcc-Modus-/SoC-Steuerung" unten), `evcc_battery_priority_enabled` (die Speicher-Prioritäts-Übersteuerung dieser Steuerung), `wallbox_min_power_w` (die Echtzeit-PV-Übersteuerung dieser Steuerung, Standard 1380 W), `evcc_realtime_override_min_solar_share` (optionaler Mindest-Solaranteil, %, für dieselbe Übersteuerung — die passende Preis-Obergrenze wird live aus evccs eigenen Tarifen berechnet, nicht als fester Preis gespeichert), `pv_forecast_today_remaining_entity` (PV-Restprognose für heute, für diese Steuerung — eine andere Entität/anderer Zeitraum als `pv_forecast_entity` in Schritt 7) sowie das optionale Haus-Nutzungsprofil über `home_consumption_entity`/`battery_charge_entity`. **Übersprungen** im Modus „Nur auswärts".
4. **Ladeleistung** (optional) — zwei unterschiedliche Quellen für zwei unterschiedliche Zwecke. Die Ladeleistung ist ein Momentanwert in W/kW, typischerweise aus der Fahrzeug-Telemetrie statt deiner Wallbox (liefert dadurch auch bei einer Fremdladung unterwegs Werte, wo die Wallbox nichts misst); sie verbessert die Energie-Schätzung einer Fremdladung gegenüber der reinen SoC-Delta-Schätzung (siehe „Energie-Schätzmethoden" oben). Der **Wallbox-Energiezähler** dagegen ist ein kumulativer kWh-Zähler deiner eigenen Wallbox (nie ein Momentanwert, nur beim Heim-Laden relevant) — er dient der automatischen Ladewirkungsgrad-Kalibrierung *und* den Heimladen-Kosten in Schritt 9. **Übersprungen** im Modus „Nur auswärts".
5. **Benachrichtigungen** (optional) — eine Benachrichtigung im Home-Assistant-eigenen Benachrichtigungsbereich erscheint bei einer erkannten Fremdladung, Fahrt oder einem Tankerkönig-Ausfall immer automatisch, unabhängig von diesem Schritt. Wähle ein oder mehrere `notify.*`-Zielgeräte (Mehrfachauswahl — Handy, Tablet, E-Mail, ...) und welche dieser Ereignisse zusätzlich einen Push (z.B. aufs Handy) auslösen sollen: Fremdladung erkannt, Fahrt erkannt, Tankerkönig nicht verfügbar und/oder SoC-Schwelle erreicht. Ist „SoC-Schwelle erreicht" ausgewählt, bestimmst du zusätzlich, bei welchen Prozentwerten (50/60/70/80/90/100 %) benachrichtigt wird — eine Benachrichtigung löst einmal pro Schwelle aus, sobald der Akku sie während eines laufenden Ladevorgangs (Heim- oder Fremdladung) erreicht.
6. **Erkennungs-Feinjustierung** — Schwellwerte der oben beschriebenen Zustandsmaschine (`start_delta`, `noise`, `idle_timeout_s`, `drop_ends`). Eine Ladung startet, sobald der SoC um `start_delta` über den letzten Ruhewert steigt; sie endet entweder bei einem SoC-Abfall um mehr als `drop_ends` unter den erreichten Höchststand, oder wenn `idle_timeout_s` lang kein neuer Höchststand erreicht wurde (z.B. Akku voll). `noise` toleriert normales Sensor-Zittern beim Verfolgen des Höchststands und muss immer **kleiner** sein als `start_delta`, sonst kann schon Rauschen eine falsche Erkennung auslösen. Die Standardwerte passen für die meisten Fahrzeuge; bei einem Auto, dessen SoC nur grob/selten aktualisiert wird (manche Cloud-APIs), großzügiger einstellen. Zwei optionale Felder, `plug_entity` (ein Stecker-/Connectivity-`binary_sensor`) und `plug_debounce_s`, bieten dafür eine direktere Alternative: ein bestätigtes „eingesteckt" überstimmt `idle_timeout_s` komplett (die Sitzung läuft einfach nie in den Timeout, solange das Auto verbunden bleibt), und ein bestätigtes „ausgesteckt" — muss zuerst `plug_debounce_s` lang anhalten, um kurze Fehlmeldungen der Quelle abzufangen — beendet die Sitzung sofort. Ein bestätigtes „ausgesteckt" verhindert außerdem, dass eine Sitzung überhaupt erst *startet*: ein SoC-Anstieg bei bestätigt ausgestecktem Fahrzeug (z.B. ein paar Punkte durch Rekuperation beim Fahren) verschiebt nur den verfolgten Referenzpunkt nach oben, statt fälschlich als Beginn einer Fremdladung gewertet zu werden. Auch ohne `plug_entity` wird eine neu erkannte Ladung mit der vorherigen noch offenen zusammengeführt, wenn dazwischen kein SoC-Abfall lag — ein Fahrzeug mit grobem SoC, das mitten in der Ladung `idle_timeout_s` auslöst, erzeugt so keine mehreren getrennten offenen Ladungen mehr.
7. **Fahrtenbuch** (optional) — Schwellwerte der Fahrten-Erkennung (`trip_min_km`, `trip_idle_timeout_s`), siehe „Fahrtenbuch" unten. Die Standardwerte passen für die meisten Kilometerstand-Sensoren. Zusätzlich eine optionale Orts-Vorschlag-Quelle (`gps_entity`, eine person-/device_tracker-/sensor-Entität — bei einem einfachen `sensor` wird dessen Zustand direkt als vorgeschlagener Ort verwendet, wenn er keiner bestehenden Zone entspricht). Ein drittes optionales Paar, `motor_entity` (ein Motor-/Fahr-`binary_sensor`, z.B. Zündung/„Ready“) und `motor_debounce_s`, gibt der Fahrten-Erkennung ein zweites Signal für Fahrzeuge, deren Kilometerstand zu grob/selten aktualisiert wird, um Fahrtbeginn/-ende allein aus der Standzeit zuverlässig abzuleiten: ein bestätigtes „fährt“ startet/verlängert eine Fahrt auch ohne frischen Kilometerstand, und ein bestätigtes „steht“ wird wie „kein Kilometerstand-Anstieg“ behandelt — `trip_idle_timeout_s` toleriert weiterhin kurze Stopps (z.B. Ampel), sodass ein kurzzeitiges Motor-Aus mitten in der Fahrt sie nicht beendet. Die Strecke stammt trotzdem immer aus dem Kilometerstand; ein echter Kilometerstand-Anstieg wird nie ignoriert, selbst wenn das Motor-Signal widerspricht. Ein weiterer optionaler Schalter, `trip_auto_confirm`, überspringt die manuelle Bestätigung komplett: eine erkannte Fahrt wird sofort ins Fahrtenbuch übernommen statt als "offene" Fahrt auf Start-/Zielort zu warten. Der Ort kommt dabei aus dem `gps_entity`-Vorschlag, falls konfiguriert, sonst bleibt er leer — nachträglich per `edit_trip` korrigierbar. Ein weiteres optionales Feld, `usage_profile_buffer_pct` (Standard 20), legt den Sicherheitspuffer auf den historischen Wochentags-Schnitt für den "morgen benötigt"-Wert im Panel-Tab „Nutzungsprofil" fest — siehe „Nutzungsprofil" unten. Ein weiteres optionales Feld, `pv_forecast_entity`, verweist auf eine beliebige Sensor-Entität mit der PV-Ertragsprognose für morgen (z.B. von Solcast oder Forecast.Solar, in kWh oder Wh) — damit konfiguriert, darf die morgen erwartete PV-Erzeugung eine Lücke schließen, die der Akku allein nicht abdeckt.
8. **Leasing** (optional) — nur aktiv, sobald **sowohl** `leasing_inkl_km` als auch `leasing_end_datum` gesetzt sind: Kilometerstand bei Vertragsbeginn (`leasing_start_km`), Vertrags-Start-/Enddatum, insgesamt inkludierte Kilometer, sowie optional ein Preis je Mehrkilometer (`leasing_preis_mehr_km`) und/oder eine Gutschrift je Minderkilometer (`leasing_preis_minder_km`). Bleibt es leer, ist das Feature komplett inaktiv — kein Sensor-Zustand, kein Panel-Inhalt.
9. **Kostenvergleich** (optional) — siehe „Kostenvergleich gegenüber einem Verbrenner" unten. Kraftstoffpreis-Priorität: Tankerkönig-Auto-Erkennung (Kraftstoffsorte wählen) > Kraftstoffpreis-Entität > fester Wert — nur eine Quelle konfigurieren. Ebenfalls optional: `co2_per_kwh_g` (Netzstrom-CO2-Intensität in g/kWh, Standard 380 — eine grobe Schätzung für den deutschen Strommix) für den unten beschriebenen Sensor „CO2-Ersparnis ggü. Verbrenner".
...
Alle Felder in Schritt 9 sind optional — beliebig viele oder wenige konfigurieren; die Sensoren unten zeigen einfach `unknown`, solange ihre nötigen Eingaben fehlen.
...
| `sensor … Fremdladung Kosten (letzte)` | `kwh × preis_kwh` für dieselbe zuletzt bestätigte Ladung, zzgl. eventueller `start_fee` (`engine.charge_cost()`). Das `historie`-Attribut (vollständige Historien-Liste) und jeder darin enthaltene Eintrag führen auch den rohen `startgebuehr`-Wert — 0 bei Einträgen, die vor Einführung dieses Felds bestätigt wurden oder bei denen keine Gebühr eingetragen wurde. |
...
| `sensor … Heimladen kWh (gesamt)` | Gesamte zuhause geladene kWh — falls verfügbar die vollständige evcc-Historie: bevorzugt evccs eigene Fahrzeug-Session-Daten, dann evccs standortweite Gesamt-Ladeenergie-Statistik (nur wenn zusätzlich ein Wallbox-Energiezähler für dieses Fahrzeug konfiguriert ist — diese Statistik ist nicht pro Fahrzeug). Fällt sonst auf die Differenz des Wallbox-Energiezählers seit Einrichtung zurück. `unknown` ohne konfigurierten Zähler. |
| `sensor … Heimladen Kosten (gesamt)` | Obige Heimladen-kWh × der Heimstrompreis aus Schritt 9 (fester Wert oder Live-Entität, siehe oben). `unknown` ohne konfigurierten Zähler oder Preis. |
| `sensor … Ersparnis ggü. Verbrenner` | Geschätzte Ersparnis gegenüber dem Vergleichs-Verbrenner aus Schritt 9, über die seit Einrichtung gefahrene Strecke (siehe „Kostenvergleich" oben). `unknown`, bis Kilometerstand-Entität, Verbrenner-Verbrauch und Kraftstoffpreis alle konfiguriert sind. Attribute: `gefahrene_km`, `heimladen_kosten`, `fremdladen_kosten`, `kosten_ev_gesamt`, `kosten_verbrenner_geschaetzt`, `kraftstoffpreis_live` (ob gerade eine Live-Quelle — Tankerkönig oder Entität — statt des festen Werts den Kraftstoffpreis liefert), `heimstrompreis_live` (dasselbe für den Heimstrompreis). |
...
| `sensor … CO2-Ersparnis ggü. Verbrenner` | Geschätzte CO2-Ersparnis gegenüber dem Vergleichs-Verbrenner, dieselbe Energiebilanz und km-Basis wie „… Fahrzeug Durchschnittsverbrauch": `(Verbrenner-Kraftstoffverbrauch × dessen CO2-Faktor) − (EV-kWh-Verbrauch × co2_per_kwh_g)`. Der Verbrenner-CO2-Faktor ist eine feste Konstante pro Kraftstoffsorte (2,33 kg/L Super, 2,24 kg/L Super E10, 2,65 kg/L Diesel — Tank-to-Wheel, ohne Vorkette/Herstellung), aus `tankerkoenig_fuel_type` übernommen, falls gesetzt, sonst Fallback auf Super/Benzin. `unknown`, bis Kilometerstand-Entität und Verbrenner-Verbrauch konfiguriert sind. Attribute: `co2_ev_kg`, `co2_verbrenner_kg`, `co2_ersparnis_kg`. |
| `sensor … Preisunterschied Fremd- vs. Heimladen` | Der tatsächlich gewichtete Durchschnittspreis für Fremdladen (Gesamt-Fremdladekosten ÷ Gesamt-Fremdlade-kWh, jeweils bestätigt) minus Heimstrompreis aus Schritt 9 — beide in €/kWh, seit Einrichtung. Positiv heißt Fremdladen war pro kWh teurer — der Normalfall. `unknown` ohne auflösbaren Heimstrompreis, oder solange noch keine Fremdladung bestätigt wurde. Attribute: `heimladen_preis_kwh`, `fremdladen_preis_kwh`, `differenz_kwh`. |
| `sensor … Kosten (heute/Woche/Monat/Jahr)` (diagnostisch) | Kombinierte Heim- + Fremdladekosten innerhalb des aktuellen Kalendertags/ISO-Woche/Monats/Jahres, mit demselben Perioden-Basislinien-und-Rollover-Mechanismus wie die Kilometer-Perioden-Sensoren unten (eine Basislinie wird einmal pro Periode beim täglichen 00:05-Refresh gesetzt, oder sofort beim ersten Start). `unknown`, bevor die erste Basislinie gesetzt ist. Anders als bei den km-Perioden-Sensoren wird ein Abfall unter die Basislinie auf 0 geklemmt statt als `unknown` angezeigt — der gewichtete Durchschnittspreis der Heimladen-Schätzung kann durch eine neue, günstigere Session leicht sinken, das ist kein Datenfehler wie ein sinkender Kilometerstand. |

### Nutzungsprofil

Beantwortet „muss ich heute nachladen, oder reicht es, bis zum PV-Überschuss von morgen zu warten?" — rein aus dem Fahrtenbuch, ohne manuelle Eingabe. `weekday_usage_profile()` in `engine.py` gruppiert `verbrauch_kwh` jeder erfassten Fahrt (falls unbekannt: `km` × `vehicle_avg_consumption` als Schätzung) nach Wochentag und bildet den Durchschnitt über die Anzahl, wie oft dieser Wochentag seit der ersten erfassten Fahrt vorkam — Tage ohne Fahrt zählen für diesen Wochentag weiterhin mit 0 kWh, sodass ein selten sonntags gefahrenes Fahrzeug den Sonntags-Schnitt korrekt nach unten zieht, statt dass der Tag übersprungen wird. Braucht mindestens 7 Tage Fahrtenbuch-Historie (`MIN_USAGE_PROFILE_DAYS`), bevor überhaupt etwas zurückgegeben wird, damit jeder Wochentag mindestens einmal beobachtet wurde.

| Key | Name | Beschreibung |
|-----|------|--------------|
| `usage_profile` | Nutzungsprofil | Durchschnittlicher kWh-Verbrauch am heutigen Wochentag. Attribute: `montag`…`sonntag` (alle 7 Wochentags-Schnitte, in kWh). `unknown` bei weniger als 7 Tagen Fahrtenbuch-Historie. |
| `usage_profile_tomorrow` | Nutzungsprofil (morgen benötigt) | Morgiger Wochentags-Schnitt zzgl. des Sicherheitspuffers `usage_profile_buffer_pct` aus Schritt 7 (Standard 20 %) — direkt mit `available_kwh` vergleichbar. Attribute: `wochentag`, `roh_kwh` (ungepuffert), `puffer_prozent`, `benoetigt_kwh` (identisch mit dem Zustand). `unknown` unter derselben Bedingung wie `usage_profile`. |
| `available_kwh` | Verfügbare kWh | Aktueller SoC (aus `soc_entity`) × nutzbare Akkukapazität aus Schritt 1. `unknown` ohne konfigurierte SoC-Quelle. |
...
| `plug_window` | Steck-Zeitfenster | Rein informativer Diagnose-Sensor: durchschnittliche Steckdauer (Stunden) je Wochentag, aus evccs eigenem Ladepunkt-Verbindungsstatus (nicht dem separaten, optionalen `plug_entity`). Attribute: `montag`…`sonntag` (nur tatsächlich beobachtete Wochentage), `heute` (laufende Zählung des heutigen Tages), `tage` (das rohe Sliding-Window je Wochentag: `date`, `stunden`, `erster_connect`, `letzter_disconnect`). Beeinflusst nicht die Modus-/SoC-Steuerung. |
| `evcc_mode_control` | evcc-Modus-Steuerung | Diagnose-Sensor für die [automatische evcc-Modus-/SoC-Steuerung](#automatische-evcc-modus-soc-steuerung) unten — der Modus (`pv`/`minpv`/`now`), auf den gerade hingesteuert wird (bereits inklusive der Echtzeit-PV-Übersteuerung unten), bzw. `unknown`, wenn diese Funktion aus ist (Standard) oder noch kein Nutzungsprofil vorliegt. Attribute: `min_soc`, `target_soc`, `verfuegbare_kwh`, `min_kwh`, `target_kwh`, `rest_heute_kwh`, `rest_heute_roh_kwh`, `pv_rest_heute_roh_kwh`, `haus_rest_heute_kwh`, `pv_fuer_auto_kwh`, `pv_ueberschuss_puffer_kwh` (heutiger PV-Überschuss, der zusätzlich gegen morgen/den Puffer angerechnet wird), `zuletzt_geschrieben`, `aktiv`, `min_soc_scope`, `limit_soc_scope`, `balancing_enabled`, `balancing_aktiv`, `naechste_vollladung_faellig_ts` (siehe [wöchentliche Vollladung](#automatische-evcc-modus-soc-steuerung) unten), `pv_override_aktiv`, `pv_ueberschuss_w` (siehe Echtzeit-PV-Übersteuerung unten), `pv_override_mischpreis_kwh` (Mischpreis dieser Übersteuerung), `pausiert` (manuelle Schreib-Pause an/aus, Service `set_evcc_mode_control_pause`/Panel-Schalter). |
| `evcc_charge_plan` | evcc-Ladeplan | Diagnose-Sensor, der evccs eigenen Ladeplan (Zielzeit-Laden, siehe Ladeplan unten) spiegelt — Zielzeit als Zustand, Attribute `target_soc`, `projected_start`, `projected_end`, `aktiv`. Unabhängig von `evcc_mode_control_enabled`, braucht nur `evcc_host`. |
| `binary_sensor … Laden vor PV empfehlenswert` | **On**, wenn `available_kwh` unter dem gepufferten Wert von `usage_profile_tomorrow` liegt — d.h. jetzt laden (z.B. aus dem Netz) ist sinnvoller, als auf den PV-Überschuss von morgen zu warten. Mit konfigurierter `pv_forecast_entity` (`_pv_forecast_tomorrow_kwh()`, rechnet Wh automatisch in kWh um) wird die morgige PV-Ertragsprognose vor diesem Vergleich zu `available_kwh` addiert (`engine.charge_before_pv_decision()`) — ein roher, nicht um den Haus-Eigenverbrauch bereinigter Wert, berücksichtigt also nicht, wie viel davon das Haus selbst verbraucht, bevor das Fahrzeug etwas abbekommt. Attribute: `verfuegbare_kwh`, `benoetigt_morgen_kwh`, `pv_prognose_morgen_kwh` (nur vorhanden, wenn `pv_forecast_entity` konfiguriert und auflösbar ist). `unknown`, wenn einer der beiden zugrundeliegenden Sensoren `unknown` ist. |

Der Panel-Tab „Nutzungsprofil" visualisiert das als Balkendiagramm (heutiger und morgiger Wochentag hervorgehoben) plus einer Klartext-Empfehlung. Darunter zeigt das Panel zusätzlich das optionale **Hausnutzungsprofil** als eigene Balkendiagramm-Karte — siehe unten.

### Automatische evcc-Modus-/SoC-Steuerung

Eine **optional zuschaltbare** Erweiterung des Nutzungsprofils oben (`evcc_mode_control_enabled` in Schritt 3, standardmäßig aus), die nicht nur eine *Empfehlung* anzeigt, sondern evccs Lademodus (`pv`/`minpv`/`now`) sowie Min-/Ziel-SoC des Fahrzeugs tatsächlich schreibt — die Wallbox folgt dem Nutzungsprofil dann ganz ohne manuelles Umschalten. Setzt einen konfigurierten `evcc_host` voraus (Schritt 3); lässt sich evccs SoC-Geltungsbereich (Loadpoint- oder Fahrzeug-Ebene — unabhängig je für Min-SoC und Ziel-SoC, manche evcc-Versionen unterstützen das eine nur auf Loadpoint-, das andere nur auf Fahrzeug-Ebene) nicht ermitteln, erscheint ein Reparieren-Hinweis — der Modus wird trotzdem gesetzt, nur diese eine SoC-Grenze nicht.

Die Entscheidung erfolgt dreistufig, wird etwa minütlich neu berechnet und **immer geschrieben, wenn sie vom tatsächlichen evccs Live-Zustand abweicht** — nicht nur von dem, was ev_assistant selbst zuletzt geschrieben hat, damit sich das selbst heilt, wenn evcc die Einstellung stillschweigend wieder verliert (z.B. nach einem evcc-Addon-Neustart, der auf evccs eigene Konfig-Defaults zurückfällt — ohne das könnte die wöchentliche Vollladung unten dauerhaft blockiert bleiben, weil das Fahrzeug nie mehr wirklich 100 % erreicht). Eine Konsequenz: ein manueller Eingriff direkt in evccs eigener Oberfläche hält jetzt nur noch bis zum nächsten Zyklus (~1 Min.), nicht mehr unbegrenzt — nutze den Service `set_evcc_mode_control_pause` (auch als Schalter direkt in der Panel-Karte "Automatische Ladesteuerung" verfügbar), um das Zurückschreiben anzuhalten, bis du ihn wieder ausschaltest — wenn eine bewusste manuelle Änderung eine Weile bestehen bleiben soll:

- **`pv`** (reine Solarladung) — der aktuelle Akkustand deckt den Rest von heute plus die nächsten zwei Tage typischen Bedarfs.
- **`minpv`** (Mindestleistung + Solar) — deckt den Rest von heute plus morgen, aber nicht den Zwei-Tage-Puffer.
- **`now`** (Netzladen erzwingen) — deckt nicht einmal morgen.

Beispiel: Nutzungsprofil sagt 8 kWh/Tag, Puffer 20 %, 45 kWh nutzbar, 40 % SoC (18 kWh) verfügbar. Ziel = (8 + 8 + 8) × 1,2 = 28,8 kWh, Min = (8 + 8) × 1,2 = 19,2 kWh → 18 kWh liegen schon unter dem Minimum, evcc bekommt also `now` sowie einen Min-SoC von rund 43 % und einen Ziel-SoC von rund 64 %.

Das Profil hinter dieser Steuerung (Fahrzeug wie Haus) nutzt ein **gleitendes Fenster der letzten Tage** statt eines Lebenszeit-Durchschnitts: je Wochentag werden nur die letzten 8 beobachteten Kalendertage vorgehalten (`USAGE_PROFILE_WINDOW_DAYS`) statt der kompletten Historie seit Einrichtung, sodass sich ein tatsächlich geändertes Nutzungsverhalten innerhalb weniger Wochen statt erst nach Jahren bemerkbar macht. Es ist außerdem **uhrzeitabhängig**: der Restbedarf für heute (`remaining_today_kwh()`) rechnet den Wochentags-Schnitt proportional zum bereits vergangenen Tagesanteil herunter, bevor der heute schon verbrauchte Wert abgezogen wird — der empfohlene Restbedarf nähert sich so im Tagesverlauf natürlich der Null an, statt nur durch reine kWh-Subtraktion gegen einen flachen Tages-Schnitt zu sinken. Die Fahrzeugseite stammt ausschließlich aus dem oben beschriebenen Live-SoC-Tracker, ohne Fahrtenbuch-Fallback für einen noch nicht beobachteten Wochentag (der gilt stattdessen konservativ als 0 kWh Bedarf).

Ein für **heute** prognostizierter PV-Überschuss, der über den heutigen Bedarf hinausgeht, reduziert zusätzlich `min_kwh`/`target_kwh` (den Bedarf von morgen/im Puffer-Fenster), nicht nur den heutigen Wert — ohne das könnte ein einzelner ungewöhnlich verbrauchsstarker Tag im Puffer-Fenster (z.B. ein geplanter Wochenendausflug) schon deutlich im Voraus nächtliches Netzladen erzwingen, selbst an einem Tag mit reichlich PV-Prognose, die das komplette Fenster locker abdecken würde. Das Attribut `pv_ueberschuss_puffer_kwh` am `evcc_mode_control`-Sensor zeigt, wie viel Überschuss so angerechnet wurde.

**Optionales Haus-Nutzungsprofil** (`home_consumption_entity`, `battery_charge_entity`, Schritt 3): ohne dieses würde eine konfigurierte `pv_forecast_today_remaining_entity` komplett dem Auto zugerechnet, obwohl davon erst der Eigenbedarf des Hauses (und eines optionalen Hausspeichers, falls dessen Ladung nicht schon im Verbrauchszähler steckt) abgeht. Ein kumulativer Hausverbrauchszähler behebt das — der typische Rest-Eigenbedarf des Hauses für heute wird zuerst von der PV-Prognose abgezogen. Wie das Fahrzeug-Nutzungsprofil baut sich auch dieses erst aus eigener gesammelter Historie auf (ein paar Tage, bis es wirkt — bis dahin zählt es stillschweigend als 0 kWh) — anders als das Fahrzeug-Profil, das volle 7 Tage braucht, bevor überhaupt etwas angezeigt wird, füllt sich die Hausnutzungsprofil-Karte im Panel aber Tag für Tag, ein noch nie beobachteter Wochentag zeigt einen gestrichelten Platzhalter statt fälschlich 0 kWh.

**Optionale wöchentliche Vollladung fürs Zellbalancing** (`weekly_full_charge_enabled`, standardmäßig aus, plus `weekly_full_charge_interval_days`, Standard 7, Schritt 3): die meisten E-Autos sind am gesündesten, wenn sie die meiste Zeit in einem mittleren SoC-Fenster verbringen — genau das strebt die profilbasierte Steuerung oben an —, aber das Batteriemanagementsystem braucht trotzdem gelegentlich eine echte Vollladung auf 100 %, um das Zellbalancing neu zu kalibrieren. Ist diese Option aktiviert und seit der letzten erreichten Vollladung (98 % gilt als „voll" — viele Fahrzeuge melden nie exakt 100 %) mindestens das konfigurierte Intervall vergangen, überstimmt die Steuerung das profilbasierte Ziel mit Ziel-SoC 100 % / Modus `minpv`, bis 100 % tatsächlich erreicht sind, danach setzt sich der Zähler zurück. Wie die 100 % erreicht wurden, spielt keine Rolle — eine manuelle Vollladung setzt den Countdown genauso zurück wie eine erzwungene, es wird also nie mehr geladen als nötig. Der Schalter lässt sich auch direkt aus der Wallbox-Karte im Panel umlegen (ein Laufzeit-Override, wie der Puffer-Regler — kein Neuladen der Integration nötig) — das Intervall selbst bleibt eine reine Options-Flow-Einstellung.
...
**Echtzeit-PV-Übersteuerung** (`wallbox_min_power_w`, Schritt 3, Standard 1380 W = 6A × 230V einphasig — bei dreiphasigem Wallbox-Anschluss wären es 6A × 3 × 230V ≈ 4140 W): Die dreistufige Entscheidung oben wird etwa minütlich aus dem Nutzungsprofil neu berechnet, das bewusst träge ist (Tages-Ebene). Darüber liegt eine zweite, schnellere Ebene, die auf den AKTUELLEN PV-Überschuss reagiert (Site-PV minus Netzeinspeisung, aus demselben Live-evcc-Feed, der ohnehin schon fürs Panel gepollt wird — kein zusätzlicher evcc-Aufruf) und "pv" in Echtzeit auf "minpv" hochstufen kann — NIEMALS umgekehrt, und nur solange das Nutzungsprofil selbst gerade "pv" empfiehlt. Das Problem, das damit gelöst wird: im reinen `pv`-Modus lädt die Wallbox erst, sobald der Überschuss ihre eigene Mindestladeleistung (`wallbox_min_power_w`) übersteigt — ein Überschuss knapp darunter (z.B. 1200 W bei 1380 W Schwelle) lädt gar nichts und geht ungenutzt ins Netz, obwohl `minpv` ihn gerne nähme (mit kleinem Zuschuss aus Netz/Speicher). Erst wenn der Überschuss die Schwelle um weitere 100 W übersteigt, schaltet es zurück auf `pv` — diese Marge (Hysterese) verhindert, dass ein Überschuss, der genau um die Schwelle pendelt (Wolken, ein kurzzeitig einschaltender Verbraucher im Haus), im Minutentakt hin- und herspringt. Bewusst zählt PV, die gerade einen Hausspeicher lädt, NICHT als "Überschuss" — die wird bereits sinnvoll genutzt (Autarkie/Zellbalancing) statt verschenkt, und eine Hochstufung dagegen würde einer in evcc selbst eingerichteten Speicherschutz-Priorisierung entgegenarbeiten.

**Optionale wirtschaftliche Kappung des Netz-Zuschusses** (`evcc_realtime_override_min_solar_share`, Schritt 3, %, kein Standardwert — ohne ihn gilt die Übersteuerung oben unverändert rein nach Watt): PV-Strom fürs Laden ist nicht wirklich kostenlos — jede nicht eingespeiste kWh kostet die entgangene Einspeisevergütung. Das Auffüllen bis `wallbox_min_power_w` mit Netzstrom hat also einen echten Mischpreis (`blended_charge_price()`): `(Überschuss × Einspeisevergütung + Netzanteil × Netzpreis) ÷ wallbox_min_power_w`. Statt diesen Preis direkt einzutragen, wird hier ein Mindest-Solaranteil (z.B. 50%) festgelegt, und die passende Preis-Obergrenze (`min_solar_share_price_ceiling()`) bei jedem Zyklus frisch aus evccs eigenen Live-Tarifen (`tariffFeedIn`/`tariffGrid`) berechnet — bleibt also automatisch korrekt, wenn sich der Tarif ändert, statt wie ein fester Preis zu veralten. Die Übersteuerung hebt nur noch auf `minpv` an, wenn der Mischpreis mindestens diesen Solaranteil bedeutet; sonst bleibt es bei `pv`, und der kleine Überschuss wird eingespeist statt teuren Netzstrom dafür zuzukaufen. Das Attribut `pv_override_mischpreis_kwh` am `evcc_mode_control`-Sensor zeigt den aktuellen Mischpreis, sobald ein `minpv`-Kandidat vorliegt — auch schon bevor eine Kappung konfiguriert ist, als Orientierung für einen sinnvollen Wert.

**Optionale Speicher-Priorität** (`evcc_battery_priority_enabled`, Schritt 3, standardmäßig aus): die Einstellung oben respektiert bewusst evccs eigene Speicher-Vorrang-Schwelle (`prioritySoc`) — aber eine fest auf 100 % stehende `prioritySoc` bedeutet, dass der Speicher an jedem PV-Überschuss-Tag IMMER zuerst komplett vollläuft, bevor das Fahrzeug überhaupt etwas bekommt. Fährt das Fahrzeug genau dann los, wenn der Speicher 100 % erreicht, bekommt es praktisch keine Solarladung ab, und jeder weitere Überschuss an dem Tag geht ungenutzt ins Netz statt es je zu erreichen. Aktiviert, senkt ev_assistant evccs site-weite `prioritySoc` (`EvccClient.async_set_priority_soc()`), während das Fahrzeug angesteckt ist — es bekommt dann vollen Vorrang vor dem Speicher beim PV-Überschuss —, und stellt die zuvor beobachtete Schwelle wieder her, sobald es abgesteckt wird, statt sie auf einen festen Wert zurückzusetzen, sodass eine direkt in evccs eigener Oberfläche gesetzte Präferenz respektiert bleibt. Ändert nur, wie PV-*Lade*-Überschuss zwischen Wallbox und Speicher verteilt wird — hat keinen Einfluss auf das *Entlade*-Verhalten des Speichers, das weiterhin komplett eurer eigenen, davon unabhängigen Steuerung (evccs eigener Batteriemodus oder eine eigene Automation) unterliegt.

### Ladeplan

Eine eigenständige, unabhängig nutzbare Ergänzung zur automatischen Modus-/SoC-Steuerung oben: setzt evccs eigenen Ladeplan (Zielzeit-Laden) über den Service `set_evcc_charge_plan` (`config_entry_id`, `target_soc`, `target_time` als Unix-Zeitstempel) oder die Panel-Karte "Ladeplan" — "das Auto soll morgen um 7 Uhr bei 80 % sein." evcc entscheidet danach selbst, WANN geladen wird. Braucht nur `evcc_host`, keine aktivierte `evcc_mode_control_enabled`. Löst einen Fall, den die profilbasierte Steuerung oben nicht abdeckt: eine harte Deadline statt eines rollierenden 1-2-Tage-Puffers.

**Solange ein Plan aktiv ist** (Sensor `evcc_charge_plan` hat einen Zustand), tritt die automatische Modus-/SoC-Steuerung komplett zurück — egal ob der Plan über ev_assistant oder direkt in evccs Oberfläche gesetzt wurde. Läuft automatisch wieder an, sobald der Plan gelöscht wird (Service `clear_evcc_charge_plan` oder Panel-Button "Plan löschen") oder von selbst ausläuft.
...
Fahrten werden automatisch aus derselben Kilometerstand-Entität erkannt, die auch für den Kostenvergleich oben genutzt wird (Schritt 1) — kein GPS nötig. Eine Fahrt ist einfach die Strecke zwischen zwei Standzeiten: sobald der Kilometerstand nach einer Standzeit wieder steigt, hat eine Fahrt begonnen; sobald er länger als der konfigurierte Timeout (Schritt 7, „Fahrtenbuch") nicht mehr steigt, wird die Fahrt abgeschlossen. Erkannte Fahrten unter der konfigurierten Mindest-Strecke werden stillschweigend verworfen (filtert Rundungsrauschen des Kilometerstand-Sensors). Das Ganze spiegelt die Architektur der Fremdlade-Erkennung (`TripDetector` neben `ChargeDetector` in `engine.py`) — dieselbe standzeit-basierte Zustandsmaschine, dieselbe neustart-sichere Persistenz.
...
- `ev_assistant.log_charge` — `config_entry_id`, `kwh`, `price_kwh` (+ optional `start_ts`, `start_fee`): eine offene Ladung bestätigen und in die Historie schreiben. **Es können mehrere Ladungen gleichzeitig offen sein** (z.B. zwei Ladestopps auf einem Roadtrip, bevor du zum Bestätigen kommst) — `start_ts` wählt die gemeinte aus; ohne Angabe wird die älteste bestätigt (FIFO). `start_fee` ist eine optionale Pauschalgebühr mancher Ladenetze/-punkte zusätzlich zum kWh-Preis (z.B. Start- oder Blockiergebühr vom Beleg), fließt in die gespeicherten Kosten ein (`engine.charge_cost()`); Standard 0.
...
- `ev_assistant.edit_charge` — `config_entry_id`, `erfasst_ts` (+ optional `kwh`, `price_kwh`, `start_fee`, `start_ts`, `end_ts`, `soc_start`, `soc_end`): korrigiert ein beliebiges Feld eines bereits bestätigten Historien-Eintrags nachträglich (z.B. ein Tippfehler, der später auffällt, oder nachträglich eingetragene SoC-Werte), identifiziert über dessen `erfasst_ts`-Attribut (siehe das `historie`-Attribut am Kosten-Sensor, oder die Historie-Liste der [EV Assistant Card](https://github.com/weskona/ev-assistant-card)) — gleiches "nur mitgegebene Felder ändern sich"-Modell wie `edit_trip`. Die laufenden Summen werden bei Änderung von kWh/Preis/Gebühr um die Differenz angepasst (nicht neu berechnet) — `kosten` wird dabei immer aus den effektiven (neuen oder bisherigen) kWh/Preis/Gebühr-Werten neu abgeleitet (`engine.charge_cost()`). Eine Änderung von `soc_start`/`soc_end` berechnet `delta_soc` neu. `end_ts` wird nicht direkt gespeichert, sondern zusammen mit dem (neuen oder bisherigen) `start_ts` in `dauer_min` umgerechnet, genau wie beim Bestätigen.
...
- `ev_assistant.edit_trip` — `config_entry_id`, `erfasst_ts` (+ optional `start_ort`, `end_ort`, `start_ts`, `end_ts`, `km`, `odo_start`, `odo_end`, `soc_start`, `soc_end`, `verbrauch_kwh`): korrigiert ein beliebiges Feld eines bereits bestätigten Fahrtenbuch-Eintrags nachträglich (z.B. ein Tippfehler, ein falsches Datum/Uhrzeit, oder eine falsch erkannte Strecke), identifiziert über dessen `erfasst_ts`-Attribut. Nur mitgegebene Felder ändern sich; eine Änderung von `km` passt die laufende Fahrtenbuch-Gesamtsumme um die Differenz an, eine Änderung von `start_ts` aktualisiert auch das `datum` des Eintrags (genutzt vom CSV-Export), und eine Änderung von `soc_start`/`soc_end` berechnet `delta_soc` und — sofern `verbrauch_kwh` nicht selbst mitgegeben wird — auch `verbrauch_kwh` neu.
...
custom_components/ev_assistant/
  ...
  evcc_client.py     # evcc-REST-Client (Lesen + evcc-Modus-/SoC-Steuerung-Schreibzugriffe)
  sensor.py
  binary_sensor.py
  services.yaml
  strings.json
...
- Home Assistant 2024.1+
- **evcc** (das Addon/Binary selbst, im Netzwerk erreichbar) — optional, aber nötig für Heimladen-Historie und Live-Energiefluss-Daten im Panel. EV Assistant spricht evccs eigene REST-API direkt an (`evcc_host` in Schritt 3) — keine separate `evcc_intg`-HA-Integration nötig.
