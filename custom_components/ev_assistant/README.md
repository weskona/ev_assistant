...
| `sensor ... External vs. Home Charging Price Difference` | The weighted-average price actually paid for external charging (total confirmed external cost ÷ total confirmed external kWh) minus the home electricity price from step 6 — both in €/kWh, since setup. Positive means external charging cost more per kWh, which is the usual case. `unknown` without a resolvable home electricity price, or before any external charge has been confirmed. Attributes: `heimladen_preis_kwh`, `fremdladen_preis_kwh`, `differenz_kwh`. |
...
The sidebar panel's "Usage Profile" tab visualizes this as a bar chart (today's and tomorrow's weekday highlighted) plus a plain-language recommendation.
...
| `sensor … Preisunterschied Fremd- vs. Heimladen` | Der tatsächlich gewichtete Durchschnittspreis für Fremdladen (Gesamt-Fremdladekosten ÷ Gesamt-Fremdlade-kWh, jeweils bestätigt) minus Heimstrompreis aus Schritt 7 — beide in €/kWh, seit Einrichtung. Positiv heißt Fremdladen war pro kWh teurer — der Normalfall. `unknown` ohne auflösbaren Heimstrompreis, oder solange noch keine Fremdladung bestätigt wurde. Attribute: `heimladen_preis_kwh`, `fremdladen_preis_kwh`, `differenz_kwh`. |
...
| `usage_profile_tomorrow` | Nutzungsprofil (morgen benötigt) | Morgiger Wochentags-Schnitt zzgl. des Sicherheitspuffers `usage_profile_buffer_pct` aus Schritt 6 (Standard 20 %) — direkt mit `available_kwh` vergleichbar. Attribute: `wochentag`, `roh_kwh` (ungepuffert), `puffer_prozent`, `benoetigt_kwh` (identisch mit dem Zustand). `unknown` unter derselben Bedingung wie `usage_profile`. |
...
Der Panel-Tab „Nutzungsprofil" visualisiert das als Balkendiagramm (heutiger und morgiger Wochentag hervorgehoben) plus einer Klartext-Empfehlung.
