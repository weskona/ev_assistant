...
- **Automatic evcc mode/SoC control from the usage profile** (opt-in, off by default via the new `evcc_mode_control_enabled` option): instead of only *recommending* whether to charge before tomorrow's solar surplus (unchanged — see `charge_before_pv_recommended`/`usage_profile_tomorrow`), ev_assistant can now actually write evcc's charge mode (`pv`/`minpv`/`now`) and the vehicle's min/target SoC, computed from the same weekday usage profile but looking further ahead (rest of today + tomorrow for the minimum, rest of today + the next two days for the target). Only writes to evcc when the computed recommendation actually changes — never on a fixed schedule — so a manual override made directly in evcc's own UI sticks around until the next real change instead of being silently reverted. Uses `POST` against evcc's REST API (verified against a live evcc 0.314.5 instance during development — its control endpoints reject `PUT` with a 404, despite older evcc docs describing `PUT`). Whether evcc exposes min-SoC and target-SoC per loadpoint or per vehicle is probed **independently for each** and cached separately (`evcc_min_soc_scope`/`evcc_limit_soc_scope`) — recent evcc versions moved min-SoC to the vehicle only while target-SoC remains settable both ways, so a single shared scope (as originally implemented) silently dropped min-SoC updates; if either probe fails, a repair issue (`evcc_soc_scope_failed`) is raised and only that one property is skipped, the mode (and the other SoC value, if its scope works) keep getting set. Vehicle-scoped writes resolve evcc's actual internal vehicle key (e.g. `db:8`) via a new `_evcc_vehicle_api_key()`, not the display title `_evcc_vehicle_key()` returns for the (unrelated) home-charging-session lookup — evcc's vehicle endpoints 400 on the title. New diagnostic sensor `evcc_mode_control` exposes the live computed values (mode, min/target SoC and kWh, PV/house breakdown, both SoC scopes) regardless of whether a write has happened yet.
- **Optional house usage profile** (`home_consumption_entity`, `battery_charge_entity`) for the control above: without it, an optionally configured "PV forecast remaining today" (`pv_forecast_today_remaining_entity` — a new, separate entity from the existing "PV forecast tomorrow" field, for a different time span) would be credited entirely to the vehicle, overstating how much solar surplus is actually left over once the house (and an optional home battery not already counted in the consumption meter) has taken its share. Deliberately kept minimal — just a per-weekday average from a single cumulative energy counter, built up the same way as the vehicle's usage profile (a few days of history before it has any effect, silently 0 kWh until then). New diagnostic sensor `house_usage_profile` exposes it (weekday averages, plus whether a home-consumption entity is configured and whether battery charging is included), and the panel's Usage Profile tab shows it as its own bar-chart card below the vehicle profile — unlike the vehicle chart, it doesn't wait for a full 7 days to show anything, and a not-yet-observed weekday renders as a dashed placeholder instead of a misleading 0 kWh bar.
...
- **New "Übersicht (Beta)" tab**, parallel to the existing Overview tab (which stays unchanged as the default) — an alternative dashboard layout for comparison, before it eventually replaces the classic one in a separate step. Built entirely from already-existing sensors/attributes, no new calculation logic: a "Kosten diesen Monat" hero card, KPI tiles (EUR/100km, savings vs. combustion, CO2 saved), a bar chart of spending over time (day/week/month/year), a cost/CO2 comparison, and the charging-location breakdown. Mode-adaptive: in "Nur auswärts" mode the status card shows the last confirmed charge instead of a live wallbox status, and instead of the home/external charging-location breakdown only the AC/DC split of external charging is shown (marked as an estimate). Cards/rows for values that don't apply are omitted entirely rather than shown empty or as 0.
- Two values (kWh this month, change vs. previous month) don't exist in the backend as their own period sensors (only cost and odometer periods are tracked) and were deliberately not newly calculated, to stay pure frontend logic — the hero and KPI row therefore only show the values that actually exist.

## [0.64.0] - 2026-08-17

### Fixed

- **A grossly underestimated charging power no longer overrides a clear SoC delta.** Field case: a detected external charge from 42% to 100% SoC (58 percentage points, plausibly ~26 kWh) was estimated at only 2.45 kWh — a factor of 10 too low. Cause: `ChargeDetector._energy()` unconditionally preferred the energy integrated from reported charging power as soon as any positive value existed at all, regardless of how large and plausible the SoC delta was. With coarse/gappy telemetry (e.g. the Stellantis app reports power and SoC only rarely and irregularly), `_integrate_power()` integrates linearly between two randomly available data points — if both happen to be low (e.g. shortly before charging starts and after the charge curve has tapered off), the genuinely high charging phase in between is completely missed.
- The SoC-based estimate is now always computed too, as soon as an SoC delta exists, and compared against the (battery-kWh normalized) power-based estimate. If the power estimate falls below 60% of the SoC estimate (`IMPLAUSIBLE_POWER_RATIO`, a new internal tunable in `const.py`), it's considered implausibly low and the SoC estimate is used instead — made visible via the new `energy_source` value `"soc_power_implausible"`, so diagnostics/history show why the power reading wasn't used. If power is at or above the SoC estimate, the previous behavior is preserved exactly — even when the SoC reading is coarse and the power reading is the more reliable, higher source, nothing gets overwritten.
- Additionally, `_integrate_power()` no longer bridges gaps of more than one hour (`MAX_POWER_GAP_S`) between two power readings linearly, but discards the contribution for that span instead — a gap that long isn't a reliable basis for assuming a given power level throughout. Only affects genuine outages/very infrequent reporting; normal reporting intervals (e.g. every 15-20 min) are unaffected.
- **Remaining limitation**: both estimators are still imperfect even with this check in place — the SoC estimate depends on the configured usable capacity (nameplate, not the actual, aging-dependent capacity), and the power estimate remains inaccurate below the 60% threshold with persistently gappy telemetry, with no third, independent source to correct it. The check only prevents a *clearly* too-low power reading from overriding a plausible SoC estimate — it doesn't make either estimate more accurate on its own.

## [0.63.1] - 2026-08-17

### Changed

- **AC/DC external-charge classification: diagnosed the underlying data, limitation now documented instead of implicit.** Checked what `dauer_min` is actually derived from in every case: for automatically detected charges it's already the pure charging-window duration (the detector ends the measurement at the last confirmed SoC/power rise, not at the later finalization — any idle time afterwards doesn't count). For manually logged or subsequently corrected charge end times, however, the duration comes from a freely entered timestamp in the panel that was previously just labeled "Ende" — if someone enters the unplug time there instead of the actual charge-end time, the resulting average power comes out too low and a genuine DC charge can be misclassified as AC. There's no more precise stored value for this case, so nothing is guessed — instead, the limitation is now named in `ac_dc_breakdown()`'s docstring and the classification is explicitly documented as an estimate. The 22 kW threshold stays unchanged, since the bias only ever runs one direction (DC→AC, never the reverse) and comes from the data entry, not from where the threshold sits.
- The two affected form fields are now labeled **"Ladeende (nicht Abstecken)"** instead of just "Ende" (manual entry and correction of an external charge), to avoid this mix-up at the source.
- The "Fremdladung nach AC/DC" section of the charging-location breakdown on the Analyse tab now itself notes that this is an estimate derived from average power, not a direct AC/DC measurement.
