...
async def test_add_maintenance_mit_preset_fuellt_defaults(hass, coordinators):
    ...
    assert punkte[0]["zeit_intervall_tage"] == 730
...
async def test_mark_maintenance_done_setzt_last_done_und_resettet_hysterese(hass, coordinators):
    ...
    # Faelligkeit dank zeit_intervall_tage=365 klar in der Zukunft (kein
    # Grenzfall wie bei einem knapp gewaehlten km-Wert).
    wartung_id = await coordinator.async_add_maintenance(
        name="Inspektion", zeit_intervall_tage=365, last_done_datum="2020-01-01",
    )
...
async def test_edit_maintenance_kriterium_entfernen_und_letztes_kriterium_schuetzen(hass, coordinators):
    ...
    wartung_id = await coordinator.async_add_maintenance(
        name="Inspektion", km_intervall=30000.0, zeit_intervall_tage=730,
    )

    # km_intervall entfernen -- zeit_intervall_tage bleibt, also erlaubt.
    ...
    assert coordinator.data["wartung"][0]["zeit_intervall_tage"] == 730

    # Auch das letzte verbleibende Kriterium entfernen -- muss verworfen werden.
    abgelehnt = await coordinator.async_edit_maintenance(wartung_id, zeit_intervall_tage="")
    assert abgelehnt is False
    assert coordinator.data["wartung"][0]["zeit_intervall_tage"] == 730  # unveraendert
...
async def test_delete_maintenance_entfernt_punkt_und_notified_eintrag(hass, coordinators):
    ...
    wartung_id = await coordinator.async_add_maintenance(name="Bremsfluessigkeit", zeit_intervall_tage=730)
    coordinator._check_wartung_thresholds()
